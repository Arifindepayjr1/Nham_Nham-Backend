...
the only schema and data scripts — no business logic, no query helpers;
migrations are immutable — once pushed to Git, never edit them;
eeds are idempotent — running them twice should not create duplicate data (use INSERT ... ON CONFLICT DO NOTHING);
eep migrations small — easier to review, easier to rollback.
